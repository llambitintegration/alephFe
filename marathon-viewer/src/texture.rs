...
pub struct LoadedCollection {
    /// RGBA pixel data for each bitmap, all resized to max_width x max_height.
...
pub struct TextureManager {
    /// Loaded collections indexed by collection number.
    pub collections: HashMap<u16, LoadedCollection>,
    /// GPU texture arrays and bind groups per collection.
...
impl TextureManager {
    ...
    /// Create GPU texture arrays and bind groups for all loaded collections.
    ...
    pub fn resolve_descriptor(&self, desc: ShapeDescriptor) -> Option<(u16, u32)> {
        ...
        let coll_idx = desc.collection() as u16;
        let shape_idx = desc.shape_index() as usize;
        let loaded = self.collections.get(&coll_idx)?;
        // shape_idx maps to a LowLevelShape, but for texture arrays we use bitmap_index.
        // Since we stored bitmaps in order, shape_idx approximates the layer.
        // For full correctness we'd need to map through LowLevelShape.bitmap_index.
        if shape_idx < loaded.bitmaps.len() {
            Some((coll_idx, shape_idx as u32))
        } else {
            None
        }
    }
...
/// Load a single collection, applying the given CLUT index.
...
/// Convert an 8-bit indexed bitmap to RGBA, transposing from column-major if needed,
/// and padding to target dimensions.
fn convert_bitmap(
    ...
    for y in 0..h.min(target_height) {
        for x in 0..w.min(target_width) {
            let src_idx = if bitmap.column_order {
                // Column-major: pixel at (x, y) is stored at x * height + y
                (x * h + y) as usize
            } else {
                (y * w + x) as usize
            };
