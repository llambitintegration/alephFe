...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Decode ShapeDescriptor: collection = bits[12:8], shape_index = bits[7:0]
    let shape_index = desc & 0xFFu;
    let light = 1.0; // TODO: read from polygon_data when storage buffer is populated

    // Determine transfer mode (use floor_transfer_mode as default for now)
