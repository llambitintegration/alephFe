...
#### Scenario: A valid Plugin.xml is parsed

WHEN a valid Plugin.xml is parsed
THEN the plugin name, description, and version are extracted
...
#### Scenario: A Plugin.xml has a minimum_version attribute

WHEN a Plugin.xml has a minimum_version attribute
THEN the required engine version is captured
...
#### Scenario: A Plugin.xml has an auto_enable attribute

WHEN a Plugin.xml has an auto_enable attribute
THEN the auto-enable flag is captured

- WHEN a Plugin.xml contains `<plugin name="P" auto_enable="false">`
- THEN the parsed plugin has auto_enable set to false

#### Scenario: Auto_enable is not specified

WHEN auto_enable is not specified
...
#### Scenario: The plugin element has no name attribute

WHEN the plugin element has no name attribute
...
#### Scenario: A plugin specifies a theme_dir

WHEN a plugin specifies a theme_dir
THEN Lua scripts and patches are cleared
...
#### Scenario: A plugin has scenario elements

WHEN a plugin has scenario elements
THEN each scenario's name, id, and version are captured
...
#### Scenario: A scenario name exceeds 31 characters

WHEN a scenario name exceeds 31 characters
...
#### Scenario: A scenario id exceeds 23 characters

WHEN a scenario id exceeds 23 characters
THEN it is truncated
...
#### Scenario: A scenario version exceeds 7 characters

WHEN a scenario version exceeds 7 characters
THEN it is truncated
...
#### Scenario: A scenario element has neither name nor id

WHEN a scenario element has neither name nor id
THEN it is skipped
...
#### Scenario: A plugin has no scenario elements

WHEN a plugin has no scenario elements
THEN it is compatible with all scenarios
...
#### Scenario: Checking compatibility against a scenario

WHEN checking compatibility against a scenario
THEN partial matches succeed
...
#### Scenario: A plugin has MML elements

WHEN a plugin has MML elements
THEN MML file paths are collected and sorted
...
#### Scenario: A plugin has a hud_lua attribute

WHEN a plugin has a hud_lua attribute
THEN the HUD Lua path is captured
...
#### Scenario: A plugin has a solo_lua element

WHEN a plugin has a solo_lua element
THEN the solo Lua path and write access flags are captured
...
#### Scenario: A plugin uses the legacy solo_lua attribute

WHEN a plugin uses the legacy solo_lua attribute
THEN it is parsed as a fallback
...
#### Scenario: A plugin has more than one solo_lua element

WHEN a plugin has more than one solo_lua element
THEN it is an error

- WHEN a Plugin.xml contains two or more `<solo_lua>` child elements
- THEN the parser logs an error and does not set solo_lua

#### Scenario: Solo_lua has no write_access children

WHEN solo_lua has no write_access children
THEN write access defaults to world
...
#### Scenario: Write_access values are parsed

WHEN write_access values are parsed
THEN all six flag types are recognized
...
#### Scenario: A plugin has a stats_lua attribute

WHEN a plugin has a stats_lua attribute
THEN the stats Lua path is captured
...
#### Scenario: A plugin has shapes_patch elements

WHEN a plugin has shapes_patch elements
THEN shapes patch paths and OpenGL flags are collected
...
#### Scenario: A plugin has sounds_patch elements

WHEN a plugin has sounds_patch elements
THEN sounds patch paths are collected
...
#### Scenario: A plugin has map_patch elements

WHEN a plugin has map_patch elements
THEN checksums and resource mappings are captured
...
#### Scenario: A map_patch resource type is a 4-character tag

WHEN a map_patch resource type is a 4-character tag
THEN it is converted to a u32 via Mac Roman encoding
...
#### Scenario: A map_patch has no checksums or no resources

WHEN a map_patch has no checksums or no resources
THEN it is skipped
...
#### Scenario: A resource type does not encode to exactly 4

WHEN a resource type does not encode to exactly 4 Mac Roman bytes
THEN the resource entry is skipped
...
#### Scenario: A directory contains a Plugin.xml file

WHEN a directory contains a Plugin.xml file
THEN a plugin is discovered at that location
...
#### Scenario: A directory contains subdirectories

WHEN a directory contains subdirectories
THEN they are scanned recursively
...
#### Scenario: A directory contains ZIP files

WHEN a directory contains ZIP files
THEN they are searched for Plugin.xml entries
...
#### Scenario: The standard Plugins directory is scanned

WHEN the standard Plugins directory is scanned
THEN plugins from all data search paths are enumerated
...
#### Scenario: Plugins are enumerated

WHEN plugins are enumerated
THEN they are sorted alphabetically by name

- WHEN plugin enumeration completes
- THEN the plugin list is sorted by plugin name in ascending alphabetical order

#### Scenario: Multiple plugins provide a HUD Lua script

WHEN multiple plugins provide a HUD Lua script
THEN only the last one in order is active
...
#### Scenario: Multiple plugins provide a stats Lua script

WHEN multiple plugins provide a stats Lua script
THEN only the last one in order is active
...
#### Scenario: Multiple plugins provide a theme

WHEN multiple plugins provide a theme
THEN only the last one in order is active
...
#### Scenario: Solo Lua plugins have conflicting write access

WHEN solo Lua plugins have conflicting write access
THEN exclusive flags determine which are active
...
#### Scenario: A plugin with world write access is active

WHEN a plugin with world write access is active
THEN it excludes all other exclusive solo Lua
...
#### Scenario: MML files from multiple valid plugins are loaded

WHEN MML files from multiple valid plugins are loaded
THEN they are applied in plugin order
...
#### Scenario: A Plugin.xml file cannot be opened

WHEN a Plugin.xml file cannot be opened
...
#### Scenario: A Plugin.xml contains malformed XML

WHEN a Plugin.xml contains malformed XML
THEN an error is logged and the plugin is skipped
...
#### Scenario: A Plugin.xml has unexpected elements or attributes

WHEN a Plugin.xml has unexpected elements or attributes
...
#### Scenario: A referenced resource file does not exist

WHEN a referenced resource file does not exist
...
#### Scenario: A referenced MML file does not exist

WHEN a referenced MML file does not exist
THEN it is excluded from the MML list
...
#### Scenario: A referenced shapes_patch file does not exist

WHEN a referenced shapes_patch file does not exist
THEN it is excluded
...
#### Scenario: A referenced sounds_patch file does not exist

WHEN a referenced sounds_patch file does not exist
THEN it is excluded
