# local-gpu-dev-harness Specification

## Purpose
A repeatable local development/investigation harness on the IO workstation that
serves the `marathon-web` WASM build and drives a Playwright/Chromium browser
against a real NVIDIA GPU adapter, for interactive WebGPU/hardware rendering
investigation. The GPU-passthrough container is the primary path (IO is
SSH-only); host-headed is optional/when a graphical session exists. Tooling +
docs only — no `marathon-web/src`, CI, or netcup production changes.
