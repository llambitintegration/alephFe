...
#### Scenario: Parsing a PXpx tag containing 208 bytes of data

WHEN parsing a PXpx tag containing 208 bytes of data
THEN the parser produces 2 physics_constants records (208 / 104 = 2), one for walking and one for running

#### Scenario: A fixed-point field contains the raw value 0x00010000

WHEN a fixed-point field contains the raw value 0x00010000 (FIXED_ONE = 65536)
THEN the parsed f32 value is 1.0

#### Scenario: A fixed-point field contains the raw value 0xFFFF0000

WHEN a fixed-point field contains the raw value 0xFFFF0000 (-65536)
THEN the parsed f32 value is -1.0

#### Scenario: A fixed-point field contains the raw value 0x00008000

WHEN a fixed-point field contains the raw value 0x00008000 (0.5 in 16.16)
THEN the parsed f32 value is 0.5

#### Scenario: The walking model has maximum_forward_velocity raw value of FIXED_ONE/14

WHEN the walking model has maximum_forward_velocity raw value of FIXED_ONE/14 (4681)
THEN the parsed f32 is approximately 0.07143

---

### Requirement: Parse monster definitions from physics WAD
...
#### Scenario: Parsing a MNpx tag containing 4680 bytes

WHEN parsing a MNpx tag containing 4680 bytes
THEN the parser produces 30 monster definitions (4680 / 156 = 30)

#### Scenario: A monster definition has flags value 0x0002

WHEN a monster definition has flags value 0x0002
THEN the monster has the _monster_flys flag set

#### Scenario: A monster has carrying_item_type of -1

WHEN a monster has carrying_item_type of -1 (NONE)
THEN the parsed value indicates no carried item

#### Scenario: Parsing attack_definition fields where type is -1

WHEN parsing attack_definition fields where type is -1 (NONE)
THEN the attack is treated as absent (no melee or ranged capability)

---

### Requirement: Parse projectile definitions
...
#### Scenario: Parsing a PRpx tag containing 1056 bytes

WHEN parsing a PRpx tag containing 1056 bytes
THEN the parser produces 22 projectile definitions (1056 / 48 = 22)

#### Scenario: A projectile has flags value 0x0010

WHEN a projectile has flags value 0x0010
THEN the _affected_by_gravity flag is set

#### Scenario: A projectile has collection value -1

WHEN a projectile has collection value -1 (NONE)
THEN the projectile is invisible (no rendered sprite)

#### Scenario: A projectile has area_of_effect of 0

WHEN a projectile has area_of_effect of 0
THEN it deals damage to a single target only

---

### Requirement: Parse effect definitions
...
#### Scenario: Parsing an FXpx tag containing 406 bytes

WHEN parsing an FXpx tag containing 406 bytes
THEN the parser produces 29 effect definitions (406 / 14 = 29)

#### Scenario: An effect has flags value 0x0001

WHEN an effect has flags value 0x0001
THEN the _end_when_animation_loops flag is set

#### Scenario: An effect has delay of 0 and delay_sound of

WHEN an effect has delay of 0 and delay_sound of -1 (NONE)
THEN the effect appears immediately with no delay sound

---

### Requirement: Parse weapon definitions including dual triggers
...
#### Scenario: Parsing a WPpx tag containing 1340 bytes

WHEN parsing a WPpx tag containing 1340 bytes
THEN the parser produces 10 weapon definitions (1340 / 134 = 10)

#### Scenario: A weapon has weapon_class value 3

WHEN a weapon has weapon_class value 3 (_twofisted_pistol_class)
THEN both primary and secondary triggers are independently active for dual-wielded operation

#### Scenario: A trigger has ammunition_type of -1

WHEN a trigger has ammunition_type of -1 (NONE)
THEN the weapon uses no consumable ammunition (e.g., fist melee)

#### Scenario: A trigger has burst_count of 0

WHEN a trigger has burst_count of 0
THEN the weapon fires single shots (no burst mode)

#### Scenario: A weapon has powerup_type of -1

WHEN a weapon has powerup_type of -1 (NONE)
THEN the weapon has no powered-up variant

---

### Requirement: Support both Marathon 2/Infinity tags and Marathon 1 tags
...
#### Scenario: A physics WAD entry contains a tag with code

WHEN a physics WAD entry contains a tag with code `MNpx`
THEN monster definitions are parsed using the Marathon 2/Infinity record layout (156 bytes per record)

#### Scenario: A physics WAD entry contains a tag with code

WHEN a physics WAD entry contains a tag with code `mons`
THEN monster definitions are parsed using the Marathon 1 record layout

#### Scenario: A physics WAD entry contains `PXpx` tagged data

WHEN a physics WAD entry contains `PXpx` tagged data
THEN player physics records are 104 bytes each with all 26 fixed-point fields

#### Scenario: A physics WAD entry contains `phys` tagged data

WHEN a physics WAD entry contains `phys` tagged data
THEN the first 100-byte record is an editor record that must be skipped, and the remaining records contain player physics constants

#### Scenario: Both Marathon 2/Infinity and Marathon 1 tags for the

WHEN both Marathon 2/Infinity and Marathon 1 tags for the same physics type are present
THEN the Marathon 2/Infinity tag takes precedence

---

### Requirement: Handle physics files with partial content
...
#### Scenario: A physics WAD entry contains only a PXpx tag

WHEN a physics WAD entry contains only a PXpx tag and no other physics tags
THEN the parser returns player physics data and indicates that monster, effect, projectile, and weapon data are absent

#### Scenario: A physics WAD entry contains MNpx and WPpx tags

WHEN a physics WAD entry contains MNpx and WPpx tags but no FXpx, PRpx, or PXpx tags
THEN the parser returns monster and weapon data and indicates the other three types are absent

#### Scenario: A physics WAD entry contains zero recognized physics tags

WHEN a physics WAD entry contains zero recognized physics tags
THEN the parser returns a result with all five physics types absent, without producing an error

#### Scenario: A tag's data length is not evenly divisible by

WHEN a tag's data length is not evenly divisible by the expected record size
THEN the parser returns an error for that specific physics type rather than silently truncating
