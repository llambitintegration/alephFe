...
- [x] 2.9 Write tests for world construction from synthetic map/physics data
...
- [x] 4.4 Implement player-wall collision response: detect wall crossings per tick, slide along walls, update polygon index
- [x] 4.5 Implement step climbing: allow crossing lines where floor height difference <= `step_delta`
- [x] 4.6 Implement ceiling collision: block entry to polygons where ceiling-floor gap < player height
- [x] 4.7 Implement media submersion: detect when player is in liquid, apply drag, deplete oxygen, apply drowning damage
...
- [x] 5.3 Implement activation cascading: propagate alerts to nearby monsters of same class
- [x] 5.4 Implement target acquisition system: select target based on enemies bitmask, distance, and line-of-sight
- [x] 5.5 Implement friendly-fire response: redirect target when damaged by a friendly entity
- [x] 5.6 Implement monster movement system: pathfind via polygon adjacency toward target, respect speed and collision
- [x] 5.7 Implement flying monster movement: 3D movement toward target at preferred hover height
- [x] 5.8 Implement monster gravity: apply gravity to non-flying monsters, track floor height
- [x] 5.9 Implement monster attack system: execute melee attacks (direct damage) and ranged attacks (spawn projectile) at defined frequency
...
- [x] 6.3 Implement burst fire: spawn `burst_count` projectiles per trigger pull with spread
- [x] 6.4 Implement dual-wielded weapons: independent left/right firing for twofisted_pistol class
...
- [x] 6.8 Implement projectile-wall collision: detect wall crossing, detonate at intersection
- [x] 6.9 Implement projectile-entity collision: check path against entity radii, apply damage on hit
- [x] 6.10 Implement homing projectile tracking: adjust velocity toward nearest target each tick
- [x] 6.11 Implement projectile range limit: despawn projectiles that exceed `maximum_range`
...
- [x] 6.14 Implement damage application: subtract from health/shield, emit damage events, handle entity death
...
- [x] 7.2 Implement platform activation: player standing, action key, monster entry, and projectile impact triggers
- [x] 7.3 Implement platform return: delay timer, then move back to resting position
- [x] 7.4 Implement platform crushing: damage entities caught between floor and ceiling, or reverse if no crush flag
- [x] 7.5 Implement platform triggers: activate linked platforms and toggle linked lights on position reached
...
- [x] 7.10 Implement item respawn: timer-based respawn in multiplayer modes
- [x] 7.11 Implement control panel activation: detect action key + facing a panel side, trigger linked platform/light/terminal
...
- [x] 8.3 Implement entity state iterator: return all active entities with position, facing, shape, and frame for rendering
- [x] 8.4 Implement event queue: level teleport, terminal activation, sound triggers, damage events
- [x] 8.5 Implement `SimWorld` serde serialization and deserialization for save/load
- [x] 8.6 Write tests for deterministic replay (two worlds with same seed produce identical state)

## 9. Integration Testing

- [x] 9.1 Create integration test: construct SimWorld from synthetic map data, advance 100 ticks with scripted input, verify player position
- [x] 9.2 Create integration test: spawn monster facing player, advance ticks, verify monster becomes alerted and attacks
- [x] 9.3 Create integration test: fire weapon, verify projectile spawns and hits wall, verify detonation
- [x] 9.4 Create integration test: activate platform, verify movement over ticks, verify platform trigger cascade
- [x] 9.5 Create integration test: pick up items, verify inventory changes, verify weapon switching
